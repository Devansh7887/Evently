...
const TicketCard = ({ ticket, event }) => {
  return (
    <div id="ticket-to-print" className="bg-white rounded-lg shadow-2xl border border-gray-200 text-left mb-6">
      {/* Ticket Header */}
      <div className="bg-gray-800 text-white p-4 rounded-t-lg">
        <h2 className="text-2xl font-bold">EVENT TICKET</h2>
        <p className="text-sm text-gray-300">Ticket ID: {ticket.ticketId}</p>
      </div>
      
      {/* Event Details */}
      <div className="p-6">
        <h3 className="text-3xl font-bold text-gray-900 mb-2">{event.title}</h3>
        <p className="text-lg text-gray-600 mb-4">{event.venue}</p>
      ...
      <div className="p-6 grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-xl font-semibold">{ticket.attendeeName}</p>
      ...
      <div className="flex justify-center p-6 bg-gray-50 rounded-b-lg">
        <img src={ticket.qrCodeUrl} alt="Booking QR Code" className="w-48 h-48" />
      </div>
    </div>
  );
};
    ...
    <div className="container mx-auto p-4 text-center max-w-2xl">
      {booking ? (
        ...
        <div className="p-6 bg-white rounded-lg shadow-lg">
          ...
          <p className="text-gray-700 mb-6">Your ticket is confirmed One copy of ticket is sent to your email ({booking.attendeeEmail}) par bhej di gayi hai.</p>
